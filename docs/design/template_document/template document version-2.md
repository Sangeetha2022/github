...
and install Live-server or (https://marketplace.visualstudio.com/items?itemName=ritwickdey.LiveServer)
 ...
 - Geppetto Backend Folder with path 
	 - Template Manager - ***/geppetto

test/application/services/templatemanager*** 
...
### 3) Important* things in below the tags should be here html template both Top and Side navigation
 ...
 - **Header tag***[It must in the body tag]
 - **Nav tag***[since,the open nav and the close nav are in the same nav tag]
 - **Footer***
...
Build our  new project
	...
	-docker ps -a
Run it in local
	-for ui buil purpose we need go like this
	-cd/project_name/application/client/desktop npm i
	-ng serve --port 0000(if port is already allocated we use this command)
