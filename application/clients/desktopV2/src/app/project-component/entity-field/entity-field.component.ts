...
export class EntityFieldComponent implements OnInit {
  ...
  constructor(private activatedRoute:ActivatedRoute,private spinner:NgxSpinnerService,private projectComponentService:ProjectComponentService,
    private regexExpression: RegexExpression,private toastr: ToastrService,   private location: Location,public dialog: MatDialog,) { 
  ...
  onCellValueChanged(event:any) {
    ...
    const index = currentEntity.findIndex((x: any) =>
      x === event.data.name);
  ...
  updateEntityField(options: boolean) {
    this.spinner.show();
    this.projectComponentService.updateEntityField(this.entity, this.logId).subscribe(
      (data:any) => {
        if (options) {
          this.spinner.hide();
