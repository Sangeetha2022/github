      ...
      ({
          defaults: Object.assign({}, defaultModel.prototype.defaults, 
          {
            ...
            script: function () 
            {
              ...
              {
                ClassicEditor.create(document.querySelector('#ckeditortextarea')).then((obj:any) => 
                {
                    console.log(obj);
                }).catch((error:any) => 
                {
          ...
          defaults: Object.assign({}, defaultModel.prototype.defaults, 
          {
            ...
            script: function () 
            {
              const initHighChart = function () 
              {
                ...
                {
                  chart: {type: '{[ charttype ]}',inverted: '{[ chartinverted ]}'},
    ...
    comps.addType(buttonName, 
    {
      ...
      ({
          ...
          handlechangetype() 
          {
            ...
            const dynamicDropdownTraits = this.get('traits').where({name: 'events'})[0];
  ...
  addGridTraits(screensVariable:any, buttonName:any) 
  {
    ...
    comps.addType(buttonName, 
    {
      ...
      ({
          ...
          script: function () 
          {
            ...
            const initAgGrid = () => 
            {
              ...
              this.gridOptions = 
              {
                ...
                rowData: rowData,
                components: {boldRenderer: function (params:any) {return '<b>' + params.value.name + '</b>';}},
        ...
        {
          // tslint:disable-next-line:max-line-length
          const gridevent = this.get('traits').where({name: 'events'})[0];
          const changedValue = this.changed['events'];
        ...
        modifier() 
        {
          const modifierObj = screensVariable.filterModifiers.find((x:any) => x.value === this.changed['modifiers']);
          if (modifierObj) 
          {
  ...
  initializeLinkMethod(screenGlobalVariable:any) 
  {
      ...
      comps.addType('link', 
      {
        ...
        ({
            ...
            {
               let temp:any='';
               temp = entityArray.find(x => x.value === this.changed['entity']);
               const entityObj = screenGlobalVariable.entityData.find((x:any) => x._id === temp.key);
               fieldArray = 
               [
                {
                  key: 'none',
            ...
            {
               let fieldTemp:any='';
               fieldTemp = fieldArray.find(x => x.value === this.changed['field']);
