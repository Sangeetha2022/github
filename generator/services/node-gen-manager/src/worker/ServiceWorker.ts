...
export class ServiceWorker {
    ...
    createService(flowDetail, gpDao, entityElement, daoObj) {
        ...
        this.gpFunction();
        console.log('final create dao values are -----   ', this.tempService);
    ...
    gpStart(daoObj) {
        ...
        if (gpStart !== undefined) {
            ...
            // this.tempService.GpStart = tempImport;
        }
        console.log(`dao worker of gpstart - count ${this.count} `, this.tempService.GpStart);
        this.count++;
    }

    gpVariableStatement(daoObj) {
        // this.tempService.GpVariable =
        //     {
        ...
        if (gpVariableStatement != undefined) {
            ...
            if (insideClassIndex < 0) {
                ...
                this.tempService.GpVariable.insideClass.push(temp);
            }
    ...
    gpCheckConnector() {
        console.log('!!@@@@@@@@@@@ -gp check connectors %%^^^^^^^^^^^  ', this.tempService);
    ...
    flowAction() {
        ...
        this.tempService.function.verbs = '';
        this.tempService.function.query = '';
        ...
        switch (this.flowDetail.actionOnData) {
            case 'GpCreate':
                ...
                this.tempService.function.verbs = `temp.save`;
                // this.tempService.method.variable = `let ${entityElement.fileName}`
                break;
            case 'GpSearch':
                this.tempService.function.methodName = this.flowDetail.actionOnData;
                this.tempService.function.parameter = `${this.entitySchema.fileName}Data, callback`;
                this.tempService.function.verbs = `this.${this.entitySchema.fileName}.findOneAndUpdate`;
                this.tempService.function.query = `{ _id: ${this.entitySchema.fileName}Data._id }, ${this.entitySchema.fileName}Data, { new: true }`;
                break;
            case 'GpUpdate':
                this.tempService.function.methodName = this.flowDetail.actionOnData;
                this.tempService.function.parameter = `${this.entitySchema.fileName}Data, callback`;
            ...
            case 'GpDelete':
                ...
                this.tempService.function.verbs = `this.${this.entitySchema.fileName}.findByIdAndRemove`;
                this.tempService.function.query = `${this.entitySchema.fileName}Id`;
                break;
            case 'GpGetAllValues':
                this.tempService.function.methodName = this.flowDetail.actionOnData;
                this.tempService.function.parameter = `callback`;
                this.tempService.function.verbs = `this.${this.entitySchema.fileName}.find`;
                break;
            case 'GpSearchDetail':
                break;
    ...
    //     if (gpVariableStatement != undefined) {
    //         this.importEntitySchema(null, tempVariable, 'variable');
    //         this.tempService.GpVariable = tempVariable;
    //     }
    //     console.log('dao worker of gpVariableStatement -----  ', gpVariableStatement, '   tempVariable   ', tempVariable);
    // }
