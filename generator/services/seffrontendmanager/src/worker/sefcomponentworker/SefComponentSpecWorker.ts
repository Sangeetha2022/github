...
export class SefComponentSpecWorker {
    generateComponentSpecFile(details, callback) {
        ...
        asyncLoop(details.desktop, async (desktopElement, next) => {
            ...
            await sefcomponentSupportWorker.handleBarsFile(templatePath, {GpHeaders}, screenGenerationPath, screenName + '.component.spec.ts');
            next();
        }, (err) => {
            if (!err) {
