...
import { SefJsonFileWorker } from './SefJsonFileWorker';
...
let sefangularJsonFileWorker = new SefJsonFileWorker();
...
export class DependencyWorker {
        ...
        if (flows.length > 0) {
            flows.map(data => {
                if (data.actionOnData == 'GpCreate' || data.actionOnData == 'GpUpdate') {
                    ...
                    sefangularJsonFileWorker.modifyAngularJsonFile(packagePath, this.angularJsonData)
                }
            })
        }
    ...
    modify_polyfills(details) {
        ...
        const polyfills = sefcomponentSupportWorker.readFile(applicationPath, (fileRes) => {
            if (fileRes.includes(`import '@angular/localize/init'`)) {
                ...
                sefcomponentSupportWorker.writeFile(applicationPath, output, (writeRes) => {
                });
            }
        });
    }
    ...
    modifyTranslateJson(details, callback) {
        ...
        asyncLoop(details.desktop, (desktopElement, desktopNext) => {
            asyncLoop(languages, (language, languageNext) => {
                ...
                sefcomponentSupportWorker.readFile(path, (fileRes) => {
                    if (fileRes) {
                        ...
                        sefcomponentSupportWorker.writeFile(path, output, (writeRes) => {
                            languageNext();
                        });
                    }
                });
