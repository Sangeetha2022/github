...
import { SefComponentCSSworker } from './sefcomponentworker/sefcomponentCSSworker';
...
import { SefComponentSpecWorker } from './sefcomponentworker/SefComponentSpecWorker';
...
let label = new Label();
let sefcomponentSupportWorker = new SefComponentSupportWorker();
const link = new Link();

const sefcomponentWorker = new SefComponentWorker();
const sefcomponentServiceWorker = new SefComponentServiceWorker();
const sefcomponentModuleWorker = new SefComponentModuleWorker();
const appModuleWorker = new AppModuleWorker();
const appRoutingModuleWorker = new AppRoutingModuleWorker();
const sefcomponetCssWorker = new SefComponentCSSworker();
const dependencyWorker = new DependencyWorker();
const sefcomponentSpecWorker = new SefComponentSpecWorker();
...
export class GenerateHtmlWorker {
    ...
    public generateComponent(details, callback) {
        sefcomponentWorker.generateComponentTs(details, (res, err) => {
            sefcomponetCssWorker.generateComponentCss(details, (res, err) => {
                sefcomponentServiceWorker.generateComponentService(details, (res, err) => {
                    sefcomponentModuleWorker.generateComponentModule(details, (res, err) => {
                        sefcomponentSpecWorker.generateComponentSpecFile(details, (res, err) => {
                            callback();
                        });
                    })
                });
    ...
    async generateHtml(gjsComponentMetadata, screensData, details) {
        ...
        this.createHtmlfromNestedObject(gjsComponentMetadata, screensData, details, (response) => {
            ...
            sefcomponentSupportWorker.writeFile(screenGenerationPath + `/${screenName.toLowerCase()}.component.html`, beautifyHtml, (writeResponse) => {
                // handle callback;
            });
        });
