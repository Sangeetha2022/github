...
import { ReactCommonWorker } from '../worker/commonWorker/reactCommonWorker';
import { Constant } from '../config/Constant';
import { Header } from '../strategy/REACT_HTML/Header';
import { GeppettoTemplateGenerator } from '../strategy/REACT_HTML/geppetto_template/GeppettoTemplateGenerator';
import { HpTemplateGenerator } from '../strategy/REACT_HTML/hp_template/HpTemplateGenerator';
import { RedHpTemplateGenerator } from '../strategy/REACT_HTML/red_template/RedTemplateGenerator';
...
import { GeppettoSideNav } from '../strategy/REACT_HTML/geppetto_template/GeppettoSideNav';
...
import { GeppettoHeader } from '../strategy/REACT_HTML/geppetto_template/GeppettoHeader';
import { GeppettoLanding } from '../strategy/REACT_HTML/geppetto_template/GeppettoLanding';
import { Footer } from '../strategy/REACT_HTML/geppetto_template/GeppettoFooter';
//SideNav path
import { TemplateSideNav } from '../strategy/REACT_HTML/generateNavigation/SideNav';
import { TemplateHeader } from '../strategy/REACT_HTML/generateNavigation/Header';
import { TemplateLanding } from '../strategy/REACT_HTML/generateNavigation/Landing';
import { TemplateGenerator } from '../strategy/REACT_HTML/generateNavigation/TemplateGenerator';
import { TemplateFooter } from '../strategy/REACT_HTML/generateNavigation/Footer';
//TopNav path
import { TemplateTopNav } from '../strategy/REACT_HTML/generateTopNavigation/TopNav';
import { TopTemplateHeader } from '../strategy/REACT_HTML/generateTopNavigation/TopNavHeader';
import { TopTemplateLanding } from '../strategy/REACT_HTML/generateTopNavigation/TopNavLanding';
import { TopTemplateFooter } from '../strategy/REACT_HTML/generateTopNavigation/TopNavFooter';
import * as ncp from 'ncp';

let reactCommonWorker = new ReactCommonWorker();
...
export class CustomReactTemplateService {
    ...
    public async customReactTemplateService(req: Request, callback: CallableFunction) {
        ...
        if (tagName === 'nav') {
            ...
            this.createHtmlfromNestedObject([gjsElement], async (res) => {
                ...
                await responseArray.forEach((item, index) => {
                    ...
                    .replace('id="template-i2uji"', ' onclick="openNav()" id="template-i2uji" ')
                ...
                const data = TemplateHeader.HTML_SIDENAV_TSX + responseArray.join('\n')  + HeaderComponent.htmlEnd[0];
    ...
    generateHtml(body, templateName) {
        ...
        let navigationType = body.template['navigation-type'];
        console.log('nav top and side', navigationType);
        ...
        if (item.hasOwnProperty('attributes')) {
            ...
            keys.forEach((key) => {
                ...
                if(item.attributes[key] !== '#') {
                    this.htmlContent = key === 'href' ? this.htmlContent + `href="${item.attributes[key]}" ` : this.htmlContent = key === '[routerlink]' ? this.htmlContent + `href="${item.attributes[key]}" ` : this.htmlContent + `${key}="${item.attributes[key]}" `;
                } else {
    ...
    public generateReactApp(details, callback) {
        ...
        dependencyWorker.generateIndexHtml(generationPath, this.projectName, this.templateName, this.grapesjsComponent, (res) => {
            ...
            reactCommonWorker.generateStyleScss(generationPath, grapesjsCSS, (res) => {
                return reactCommonWorker.generateMainFile(generationPath, details, this.sharedObj, this.projectName, (response) => {
