...
export class CustomTemplateService {
    ...
    public createCustomTemplate(req: Request, callback: CallableFunction) {
        ...
        this.exec(`cd ${this.generationPath.replace(/\s+/g, '\\ ')} && ng new ${this.projectName} --routing=false --skip-git --style=scss --skip-install --strict`, (error, stdout, stderr) => {
            ...
            if (stdout || stderr) {
                ...
                this.generateCustomApp(this.details, (response) => {
    ...
    public generateCustomApp(details, callback) {
