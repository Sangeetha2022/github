...
export class CommonSupportWorker {
  ...
  async generateSefServerFile(generationPath, templatePath, serverData, callback) {
    ...
    generateServiceApiFile(generationPath, templatePath) {
        const configPath = `${generationPath}/src/config`;
        const serviceApiTemplatePath = path.resolve(__dirname, templatePath);
        Common.createFolders(configPath);
        let generateServiceApi = st.loadGroup(require(serviceApiTemplatePath + '/SystemEntryService_stg'));
        let serviceApiFile = generateServiceApi.render("SystemEntryService");
        fs.writeFile(configPath + `/SystemEntryService.ts`, serviceApiFile, function (err) {
            if (err) throw err;
            console.log('SystemEntryService files generated');
        })
    }
