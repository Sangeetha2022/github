...
import { ServiceWorker } from '../sefworker/SefServiceWorker';
import { ControllerWorker } from '../sefworker/SefControllerWorker';
import { DaoWorker } from '../sefworker/SefDaoWorker';
import { RouteWorker } from '../sefworker/SefRouteWorker';
...
import { CommonWorker } from '../sefworker/CommonWorker';
import * as Constants from '../config/Constants';
import { AttachmentWorker } from '../sefworker/AttachmentWorker';
import { ExternalFeatureWorker } from '../sefworker/externalfeatureWorker';
...
export class SefNodeService {

    // constant
    private SEF_NODE_MICROSERVICE = 'Sef Node Microservice';
    ...
    public async createProjectSefNode(req: Request, callback) {
        ...
        // if(details.featureName == 'SystemEntryFeature'){
        ...
        if (EntitySchema == undefined && !Object.values(details.externalfeature).includes('external')) {
            console.log('-----coming inside the if condition on the sef node generation---');

            callback('No Schema has been found');
        } else if (Object.values(details.externalfeature).includes('external')) {
            console.log('--------sef node gen--feature------', details.externalfeature);
                ...
                asyncLoop(EntitySchema, async (entityElement, entityNext) => {
                    ...
                    if (entityElement === undefined) {
                        ...
                        asyncLoop(details.flows, async (flowElement, flowNext) => {
                            ...
                            const controller = controllerWorker.createSefController(tempFlow, gpController, entityElement, this.controllerObj);
                            const service = serviceWorker.createSefService(tempFlow, gpService, entityElement, this.serviceObj, modifierResponse);
                            const dao = daoWorker.createSefDao(tempFlow, gpDao, entityElement, this.daoObj, modifierResponse);
                            const route = routeWorker.createSefRoutes(tempFlow, entityElement, this.routeObj);
                    ...
                    if (entityError) {
                        ...
                        controllerWorker.generateSefControllerFile(projectGenerationPath, templateLocation, this.controller);
                        serviceWorker.generateSefServiceFile(projectGenerationPath, templateLocation, this.service);
                        daoWorker.generateSefDaoFile(projectGenerationPath, templateLocation, this.dao);
                        routeWorker.generateSefRouteFile(projectGenerationPath, templateLocation, this.route);
                        ...
                        this.packageObj.description = `${featureName} ${this.SEF_NODE_MICROSERVICE}`;
                        commonWorker.generatePackageJsonFile(projectGenerationPath, templateLocation, this.packageObj);
                        commonWorker.createSefServerFile(projectGenerationPath, templateLocation, projectName, port);
                        ...
                        commonWorker.generateServiceApiFile(projectGenerationPath, templateLocation);
                      //apigatewaySupportWorker.generateApiAdapter(projectGenerationPath + '/src/config', templateLocation+'/apigateway');
                        console.log('test sef-node response', this.route);
                        callback(this.route);
                    }
                });
            } catch (error) {
