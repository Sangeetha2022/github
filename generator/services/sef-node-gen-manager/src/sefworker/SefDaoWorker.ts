...
import { DaoSupportWorker } from '../sefsupportworker/SefDaoSupportWorker';
...
export class DaoWorker {
    ...
    createSefDao(flowDetail, gpDao, entityElement, daoObj, modifierResponse) {
    ...
    generateSefDaoFile(projectGenerationPath, templateLocationPath, dao) {
        dao.forEach(daoElement => {
            console.log('dao element values are -------- ', util.inspect(daoElement, { showHidden: true, depth: null }));
            daoSupportWorker.generateSefDaoFile(projectGenerationPath, templateLocationPath, daoElement, (response) => {
